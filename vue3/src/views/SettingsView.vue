...
import { prefs, workspaces, persist, resetAll, uploadImage, fetchUploads, deleteUpload, uploadUrl, DEFAULT_ICP_URL } from '../store/useStore'
...
const bgOptions = computed(() => {
  ...
  uploads.value.forEach(u => {
    // name 是「身份」（存进 prefs.bgUrl、用来比对），url 只是显示用的地址。
    // 存裸文件名而不是 /api/uploads/x.jpg，跨源部署时才不会裂。
    opts.push({ key: 'upload:' + u.name, type: 'upload', n: u.name, name: u.name, thumb: uploadUrl(u.name) })
  })
  return opts
})
...
function isApplied(o) {
  ...
  return prefs.bg === 'custom' && prefs.bgUrl === o.name
}
function isSelected(o) {
  if (!selected.value) return isApplied(o)
  if (o.type !== selected.value.type) return false
  return o.type === 'preset' ? o.v === selected.value.v : o.name === selected.value.name
}
function choose(o) {
  selected.value = o.type === 'preset' ? { type: 'preset', v: o.v } : { type: 'upload', name: o.name }
}
function applyBg() {
  if (!selected.value) { toast('请先选择一张背景图'); return }
  if (selected.value.type === 'preset') { prefs.bg = selected.value.v; prefs.bgUrl = '' }
  else { prefs.bg = 'custom'; prefs.bgUrl = selected.value.name }
  persist()
  toast('背景已更换')
}
...
async function onUpload(e) {
  ...
  try {
    ...
    await loadUploads()
    selected.value = { type: 'upload', name: up.name } // 自动选中新上传的
...
async function removeUpload(o) {
  ...
  try {
    ...
    if (prefs.bg === 'custom' && prefs.bgUrl === o.name) {
...
async function saveKey() {
  ...
  try {
    await changeKey(k)     // 服务端用新密钥重算加盐校验值（密钥本身不落盘）
    newKey.value = ''
    keyError.value = ''
    toast('访问密钥已更新')
  } catch (e) {
...
onMounted(async () => {
  await loadUploads()
  // prefs.bgUrl 存的就是裸文件名（applyConfig 已把历史值归一化）
  selected.value = (prefs.bg === 'custom' && prefs.bgUrl)
    ? { type: 'upload', name: prefs.bgUrl }
...
// ── 页脚备案信息 ──
// 开关控制是否展示；链接留空或没写协议头都归一化，回落到默认的工信部地址
function toggleIcp() {
  prefs.icpOn = !prefs.icpOn
  persist()
  toast(prefs.icpOn ? '已开启页脚备案展示' : '已关闭页脚备案展示')
}
function saveIcp() {
  prefs.icpText = String(prefs.icpText || '').trim()
  let url = String(prefs.icpUrl || '').trim()
  if (url && !/^https?:\/\//i.test(url)) url = 'https://' + url.replace(/^\/+/, '')
  prefs.icpUrl = url || DEFAULT_ICP_URL
  persist()
  toast('备案信息已保存')
}
...
async function doResetAll() {
  ...
  const ok = await resetAll()   // 服务端重置为 DEFAULT_CONFIG 并回填；失败时自己已弹提示
  if (!ok) return
...
<template>
  <div class="page-settings">
      ...
      <div class="cards">
        <section class="panel">
          ...
          <div class="save-hint">
            ...
            点选背景图后按「确定更换背景」生效；带 ✓ 的是当前使用中的。上传的图片存于服务端的图片目录（磁盘文件，不进数据库）。
          </div>
        ...
        <section class="panel">
          ...
          <div ref="wsList" class="ws-list">
            <div v-for="(ws, idx) in workspaces" :key="ws.id" class="ws-row">
              ...
              <div class="ws-edit">
                ...
                <input class="input path" :value="ws.root" placeholder="后端机器上的路径" aria-label="路径" @change="onWsField(ws, 'root', $event)" />
              </div>
          ...
          <div class="inline-tip">
            ...
            <span>每个工作区对应<strong>后端所在机器</strong>上的一个目录：标注其系统环境（Windows / Linux）并填入真实路径。后端会实时扫描该路径，读取目录树、md 正文与引用图片。删除某个工作区后，博客右上角的分区中就不再显示它。<br />
            服务部署在服务器上时，这里要填<strong>服务器上</strong>的路径（如 <code>/srv/blog/mymajor</code>），填本机 Windows 路径服务器读不到。</span>
          </div>
          <div class="add-row">
        ...
        <section class="panel">
          ...
          <div class="save-hint">
            <svg class="icon-sm" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7"><path d="M5 13l4 4L19 7"/></svg>
            所有改动自动保存到服务端数据库（H2），返回博客后立即生效。
          </div>
        </section>

        <section class="panel">
          <div class="panel-title"><h2>备案信息</h2><span>ICP FILING</span></div>
          <div class="chip-group">
            <button type="button" class="chip" :class="{ on: prefs.icpOn }" @click="toggleIcp">在页脚展示备案信息</button>
          </div>
          <div class="row2" style="margin-top:18px;">
            <div class="field">
              <label for="icp-text">备案展示名</label>
              <input
                id="icp-text"
                class="input"
                v-model="prefs.icpText"
                maxlength="60"
                spellcheck="false"
            ...
            <div class="field">
              <label for="icp-url">跳转链接</label>
              <input
                id="icp-url"
                class="input mono"
                v-model="prefs.icpUrl"
                maxlength="300"
                spellcheck="false"
                autocomplete="off"
                :placeholder="DEFAULT_ICP_URL"
                @change="saveIcp"
              />
            </div>
          </div>
          <div class="save-hint">
            <svg class="icon-sm" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7"><path d="M5 13l4 4L19 7"/></svg>
            <span>
              开关打开<strong>且</strong>填了展示名，页脚里才会出现备案号 —— 与左侧署名<strong>同一行、靠右</strong>（默认不展示）。点击备案号在新窗口打开跳转链接，留空则用
              <span class="mono">{{ DEFAULT_ICP_URL }}</span>；只填域名也行，会自动补 <span class="mono">https://</span>。
            </span>
          </div>
        </section>
    ...
    <SiteFooter label="open markdown blog — 设置页" note="workspace-blog + settings · 同一份本地设置" />
  </div>
</template>
