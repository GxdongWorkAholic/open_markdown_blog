// Markdown 渲染器 —— 本地图片经后端 /api/img 提供，外部图片直出
import { API_BASE } from '../store/http'
...
// 本地图片 URL：由后端按「工作区 id + 工作区内相对路径」提供。
// 传 id 而不是绝对路径 —— 客户端不掌握、也无法指定服务器上的任意目录。
function imgUrl(wsId, rel) {
  return API_BASE + '/api/img?ws=' + encodeURIComponent(wsId) + '&rel=' + encodeURIComponent(rel)
}

// 规范化 md 正文里图片的相对引用路径（处理 ../ 与 .），返回相对工作区根的路径
...
function inlineMd(txt, docDir, wsId) {
  let s = esc(txt)
  s = s.replace(/!\[([^\]]*)\]\(([^)\s]+)(?:\s+"[^"]*")?\)/g, function (m, alt, url) {
    ...
    if (imgRel) {
      return '<figure><img class="md-img" src="' + imgUrl(wsId, imgRel) + '" alt="' + esc(alt || '') + '" loading="lazy"><figcaption>' + esc(alt || '') + '</figcaption></figure>'
    }
...
export function mdToHtml(md, docDir, wsId) {
  const headings = []
  ...
  function flushPara() {
    if (para.length) { out.push('<p>' + inlineMd(para.join(' '), docDir, wsId) + '</p>'); para = [] }
  }
  function flushTable() {
    if (!tableRows.length) return
    const head = tableRows.shift()
    let html = '<div style="overflow:auto;"><table><thead><tr>' +
      head.split('|').filter(c => c.trim() !== '').map(c => '<th>' + inlineMd(c.trim(), docDir, wsId) + '</th>').join('') +
      '</tr></thead><tbody>'
    tableRows.forEach(r => {
      html += '<tr>' + r.split('|').filter(c => c.trim() !== '').map(c => '<td>' + inlineMd(c.trim(), docDir, wsId) + '</td>').join('') + '</tr>'
    })
  ...
  for (i = 0; i < lines.length; i++) {
    ...
    if ((m = /^(#{1,6})\s+(.*)$/.exec(trim))) {
      ...
      out.push('<h' + lvl + ' id="' + id + '">' + inlineMd(txt, docDir, wsId) + '</h' + lvl + '>')
      continue
    }
    ...
    if (/^&gt;\s?/.test(trim) || /^>\s?/.test(trim)) {
      ...
      i--
      out.push('<blockquote>' + inlineMd(qs.join(' '), docDir, wsId) + '</blockquote>')
      continue
    }
    if ((m = /^(\s*)([-*+]|\d+\.)\s+(.*)$/.exec(ln)) && m[3].length) {
      ...
      out.push('<li>' + inlineMd(m[3], docDir, wsId) + '</li>')
      continue
    }
