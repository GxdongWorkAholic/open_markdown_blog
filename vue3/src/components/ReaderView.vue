...
const rendered = computed(() => {
  ...
  // 第三个参数是工作区 **id**（不是绝对路径）——后端据此查根目录并做越界校验
  return mdToHtml(docText.value, docDir.value, wsMeta.value ? wsMeta.value.id : '')
})
...
// 顶上显示「工作区名 · 路径」：同一篇笔记在开发机与服务器上的路径完全不同，光看名字认不出来
const docMeta = computed(() => (wsMeta.value ? wsMeta.value.name + ' · ' + wsMeta.value.root : ''))
