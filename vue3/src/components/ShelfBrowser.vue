...
import {
  workspaces, activeWs, activePath, q, tree, loading, error, configError,
  setActiveWs, openFolder, openDoc, loadTree,
    ...
    <div class="container">
      ...
      <div v-if="workspaces.length" class="browser">
        <aside class="tree-pane">
          ...
          <div v-else-if="error" class="empty" style="padding:24px 10px;">
            {{ error }}
            <div style="margin-top:10px;"><button type="button" class="btn btn-ghost" @click="loadTree">重试</button></div>
          </div>
      ...
      <div v-else class="empty" style="margin-top:var(--gap-lg);border:1px dashed var(--border);border-radius:16px;padding:60px 24px;">
        <template v-if="configError">
          {{ configError }}<br />
          已配置的工作区没能从服务端读出来，请先确认本地目录服务已启动，再刷新本页。
        </template>
        <template v-else>
